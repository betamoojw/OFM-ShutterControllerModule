        ...
        else
        {
            if (ko.value(DPT_Step))
            {
                ...
                if (_targetSlatPosition != 0)
                {
                    ...
                    logDebugP("Erhöhen Slat: %d", (int)_targetSlatPosition);
                }
                else if (_targetPosition != 0)
                {
                    targetPosition = max(_targetPosition - 2, 0);
                    logDebugP("Erhöhen Position: %d", (int)targetPosition);
                }
            }
            else
            {
                ...
                if (_targetSlatPosition != 100)
                {
                    ...
                    logDebugP("Verringern Slat: %d", (int)_targetSlatPosition);
                }
                else if (_targetPosition != 100)
                {
                    targetPosition = min(_targetPosition + 2, 100);
                    logDebugP("Verringern Position: %d", (int)targetPosition);
                }
            }
        }
        break;
...
void ShutterSimulation::update(const CallContext &callContext)
{
    ...
    if (_lastPositionChange != 0)
    {
        ...
        {
            if (callContext.currentMillis - _lastPositionChange > (_positionController.simulationMode() == SimulationMode::FastSimulation ? 80 : 800))
            {
                if (_targetPosition > _currentPosition)
                {
                    _currentPosition++;
                ...
                if (_targetPosition != _currentPosition)
                {
                    if (abs((uint8_t)KoSHC_CShutterPercentInput.value(DPT_Scaling)) - _currentPosition >= 10)
                    {
                        ...
                        KoSHC_CShutterPercentInput.value(_currentPosition, DPT_Scaling);
                        logDebugP("Send Position: %d", (int)_currentPosition);
                    }
                    setMoving(true);
                }
                else
        ...
        if (_lastGroupObjectUpdate != 0 && callContext.currentMillis - _lastGroupObjectUpdate > (_positionController.simulationMode() == SimulationMode::FastSimulation ? 1000 : 10000))
        {
            ...
            if (callContext.hasSlat)
            {
                if (KoSHC_CShutterSlatInput.commFlag() != ComFlag::WriteRequest)
                    if (KoSHC_CShutterSlatInput.valueCompare(_currentSlatPosition, DPT_Scaling))
                        logDebugP("Send Slat: %d", (int)_currentSlatPosition);
            }
            _lastGroupObjectUpdate = callContext.currentMillis;
        }
    }
